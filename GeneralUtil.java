...
public class GeneralUtil {	
	public static String path = "\\G:\\GeneralFramework\\";
	...
	public static String dataStreamForFlowSize = "\\G:\\GeneralFramework\\CAIDA\\output"; // "\\D:\\CAIDA\\traffic\\traffic.txt";		//"\\D:\\CAIDA\\final_result_1\\output.txt";  /* path + "traffic\\CAIDA_data0_statistics_size.txt";*/
	public static String dataSummaryForFlowSize = "\\D:\\CAIDA\\final_result_1\\outputsrcDstCount.txt"; //path + "traffic\\CAIDA_data0_statistics_size.txt";
	public static String dataStreamForFlowSpread = "\\G:\\GeneralFramework\\CAIDA\\output"; //path + "traffic\\10MPackets.txt";
	...
	public static String dataSummaryForFlowSpread = "\\D:\\CAIDA\\largespread\\outputdstcard.txt";
	//public static String dataSummaryForFlowSpread = "\\D:\\CAIDA\\final_result_1\\output2.txt";
	public static String dataStreamForFlowThroughput = "\\D:\\CAIDA\\final_result_1\\output1.txt";
	...
	public static String dataStreamForJointFlowSize = "\\D:\\GF-EXP\\traffic\\UF\\processed\\ctx36-nexus\\output201707120000.txt"; // "\\D:\\CAIDA\\traffic\\traffic.txt";		//"\\D:\\CAIDA\\final_result_1\\output.txt";  /* path + "traffic\\CAIDA_data0_statistics_size.txt";*/
	public static String dataSummaryForJointFlowSize = "\\D:\\GF-EXP\\traffic\\UF\\temporal_join\\src5TsJointSize20170712.txt"; //path + "traffic\\CAIDA_data0_statistics_size.txt";
	public static String dataStreamForJointFlowSpread =  "\\D:\\GF-EXP\\traffic\\UF\\processed\\ctx36-nexus\\output201707120000.txt"; //path + "traffic\\10MPackets.txt";
	public static String dataSummaryForJointFlowSpread = "\\D:\\GF-EXP\\traffic\\UF\\temporal_join\\src5TsJointSpread20170712.txt";//path + "traffic\\10MCardinality.txt";
	...
	//public static ArrayList<Integer> bin = new ArrayList<>(Arrays.asList(10, 50, 100, 500, 1000,2000, 5000, 10000, 20000, 30000,40000,60000,80000,100000,160000));
	public static ArrayList<Integer> bin = new ArrayList<>(Arrays.asList(1, 2, 4, 8, 16,32, 64, 128, 256,512,1024, 2048,4096,8192,16384,32768,65536,131072,262144, 524288, 1048576,1000000000));
	//public static ArrayList<Integer> bin = new ArrayList<>(Arrays.asList(50, 100, 500, 1000, 3000, 10000,20000));
	public static ArrayList<Integer> smallBin = new ArrayList<>(Arrays.asList(1, 2, 4, 8, 16,32, 64, 128, 256,512,1024, 2048,4096,8192,16384,32768,65536,131072));
...
//		else if (num <= 100) return 0.5;
	...
	public static void analyzeAccuracy(String filePath) throws FileNotFoundException {
		...
		while (sc.hasNextLine()) {
			...
			if ((int)((shat-s)/s*100)/100.00<=epsilon) epsilonRatio[binIndex] +=1.0;
	...
	public static void analyzeAccuracywithoutnoise(double[] noise,String filePath) throws FileNotFoundException {
		//initBin();
		analyzeSmallAccuracy(filePath);
		int binLen = bin.size();
		...
		PrintWriter pw1 = new PrintWriter(new File(filePath + "_Bias"));
		PrintWriter pw2 = new PrintWriter(new File(filePath + "_Standard_Error"));
		PrintWriter pw5 = new PrintWriter(new File(filePath + "_bias_nonoise"));
		PrintWriter pw3 = new PrintWriter(new File(filePath + "_abs_Error"));
		Scanner sc = new Scanner(new File(filePath));
		double tt=0.0;
		int[] mid=new int[bin.size()];
		mid[0]=bin.get(0)/2;
		for(int i=1;i<bin.size();i++)
			mid[i]=(bin.get(i-1)+bin.get(i))/2;
		while (sc.hasNextLine()) {
			String entry = sc.nextLine();
			String[] strs = entry.split("\t");
						
			int s = Integer.parseInt(strs[strs.length-2]);			//true value
			double shat = (double)Integer.parseInt(strs[strs.length-1]);		//estimated value
			
			int binIndex = getBinIndex(s);
			int kk=0;
			int kk=binIndex;
			while(true) {
				int kkk=getBinIndex((int)shat-(int)noise[kk]);
				if(kkk==kk)
					break;
				kk=kkk;
			}
			while(kk<bin.size()-1&&(int)noise[kk]+bin.get(kk)<shat) kk++;
			shat-=(int)noise[kk];
			shat=Math.max(1, shat);				
			num[binIndex]++;
			totalNum++;


			noise[binIndex] += shat-s;
			bias[binIndex] += 1.0*(shat - s) / s;
			totalBias += Math.abs(shat-s);
			stderr[binIndex] += (1.0 * shat / s - 1) *  (1.0 * shat / s - 1);
			tt+=Math.abs(1.0*(shat - s) / s);
		}
		sc.close();
		...
		for(int j = 0; j < binLen; j++) {
			if(num[j] != 0) {
				...
				pw1.println(bin.get(j) + "\t" + avg[j] + "\t" + num[j]+ "\t" + bias[j]);
				pw2.println(bin.get(j) + "\t" + avg[j] + "\t" + num[j]+ "\t" + + stderr[j]);
				pw5.println(bin.get(j) + "\t" + num[j]+ "\t" + + bias[j]);
				pw3.println(bin.get(j) + "\t" + avg[j] + "\t" + num[j]+ "\t" + + abserr[j]);
			}
		}
		System.out.println(filePath + ":\t" + totalBias/totalNum);
		System.out.println("Rerr" + ":\t" + tt/(double)totalNum);
		pw1.close();
		pw2.close();
		pw5.close();
